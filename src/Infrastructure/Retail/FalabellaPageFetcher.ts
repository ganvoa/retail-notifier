...
export class FalabellaPageFetcher implements PageFetcher {
    ...
    async getPage(limit: number, offset: number, page: number): Promise<string> {
        const url = `https://www.falabella.com/s/browse/v1/listing/cl?f.derived.variant.sellerId=FALABELLA&facetSelected=true&page=${page}&categoryId=${this.department.slug.split('/')[0]}&categoryName=${this.department.slug.split('/')[1]}`;
