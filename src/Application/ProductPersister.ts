...
export class ProductPersister implements ProductHandler {
  constructor(private broker: FanoutBroker, private repository: ProductRepository) {}

  async handle(product: Product) {
    if (product.shouldStore) {
      ...
      if (productAlreadyExists == undefined) {
        console.log(
          `storing product: ${product.discountPercentage};${product.shouldNotify};${product.retailId};${product.name};${product.productUrl}`
        );
